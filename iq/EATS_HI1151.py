...
class IQxel():
    ...
    def vsa(self, mw):
        ...
        if mode == '11b':
            ...
            standard = 'OFDM'
            mask_limit = 'AUTO'
        elif mode == '11ng20' or mode == '11na20' or mode == '11ng40' or mode == '11na40':
...
class DUT:
    ...
    def login(self, host, username, password):
        ...
        self.tn.read_until(b'ogin:', timeout=1)
    ...
    def upc_write(self):
        ...
        self.tn.write(b'wifi calibrate test parameter write wifichip %s type upc len %s value %s\r\n' %
                      (band.encode('ascii'), upc_length.encode('ascii'), pwrcali_para.encode('ascii')) + b'\r\n')
        logger.debug(
            f'wifi calibrate test parameter write wifichip {band} type power len {upc_length} value {pwrcali_para}')
        command_result = self.tn.read_all()
        for result in command_result:
            logger.debug(result.strip().decode('utf-8'))
    ...
    def tx(self):
        ...
        # time.sleep(2)
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'iwpriv vap%s setessid Hi' % vap.encode('ascii') + b'\n')
        self.tn.read_until(b'# ', timeout=1)
        self.tn.write(b'iwpriv vap%s mode %s\r\n' % (vap.encode('ascii'), mode.encode('ascii')))
        # logger(channel)
        if bw == '40':
        ...
        # logger(channel)
        self.tn.read_until(b'#  ', timeout=1)
        self.tn.write(b'iwpriv vap%s freq %s\r\n' % (vap.encode('ascii'), channels.encode('ascii')))
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'iwpriv vap%s privflag 1' % vap.encode('ascii') + b'\n')
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'hipriv.sh "vap%s 2040bss_enable 0"' % vap.encode('ascii') + b'\n')
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'hipriv.sh "vap%s radartool enable 0"' % vap.encode('ascii') + b'\n')
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'hipriv.sh "vap%s acs sw 0"' % vap.encode('ascii') + b'\n')
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'hipriv.sh "vap%s add_user 07:06:05:04:03:02 1"\n' % vap.encode('ascii'))
        self.tn.read_until(b'# ', timeout=1)
        self.tn.write(b'iwpriv vap%s bw %s\r\n' % (vap.encode('ascii'), bw.encode('ascii')))
        ...
        self.tn.read_until(b'# ', timeout=1)
        self.tn.write(
            b'iwpriv vap%s %s %s\r\n' % (vap.encode('ascii'), rate_para.encode('ascii'), rates.encode('ascii')))
        if chain == '0':
            chains = '01'
        else:
            chains = '10'
        ...
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'iwpriv vap%s txch 00%s\r\n' % (vap.encode('ascii'), chains.encode('ascii')))
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'iwpriv vap%s al_tx 1\r\n' % vap.encode('ascii'))
        # self.tn.read_until(b'#', timeout=1)
        # self.tn.write(b'hipriv.sh "vap%s set_tx_pow rf_reg_ctl 0"' % vap.encode('ascii') + b'\n')
        time.sleep(1)
    ...
    def tx_off(self):
        ...
        self.tn.write(b'iwpriv vap%s al_tx 0' % vap.encode('ascii') + b'\n')
    ...
    def get_paras(self):
        ...
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'hipriv.sh "vap%s set_tx_pow rf_reg_ctl 1"' % vap.encode('ascii') + b'\n')
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'hipriv.sh "vap%s reginfo soc 0x20038%s 0x20038%s";dmesg -c'
                      % (vap.encode('ascii'), channel_groups.encode('ascii'), channel_groups.encode('ascii')) + b'\n')
    ...
    def adjust_power(self):
        ...
        self.tn.write(b'hipriv.sh "vap%s regwrite soc 0x20038%s 0x%s"\r\n'
                      % (vap.encode('ascii'), channel_groups.encode('ascii'), pwr_paras.encode('ascii')))
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'hipriv.sh "vap%s reginfo soc 0x20038%s 0x20038%s";dmesg -c\r\n'
                      % (vap.encode('ascii'), channel_groups.encode('ascii'), channel_groups.encode('ascii')))
        command_result = self.tn.read_until(b'value=0x\w+\r\r\n#', timeout=1)
        command_result = re.search(b'value=0x\w+', command_result)
        default_value = re.sub('value=0x', '', command_result.group().decode('ascii'))
    ...
    def set_default(self):
        ...
        self.tn.read_until(b'SU_WAP>', timeout=1)
        self.tn.write(b'wifi calibrate test parameter write wifichip %s type power len %s value default\r\n' %
                      (band.encode('ascii'), length.encode('ascii')))
        time.sleep(2)
        self.tn.read_until(b'SU_WAP>', timeout=1)
        self.tn.write(b'shell' + b'\n')
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'wifi_equipment.sh init chip %s\r\n' % band.encode('ascii'))
        time.sleep(2)
        self.tn.read_until(b'#', timeout=1)
        self.tn.write(b'hipriv.sh "vap%s set_tx_pow rf_reg_ctl 0"\r\n' % vap.encode('ascii'))
    ...
    def rx(self):
        ...
        # logger('chain set',chain,type(chain),chains)
        self.tn.read_until(b'root@OpenWrt:~# ', timeout=1)
        self.tn.write(b'iwpriv vap%s rxch 00%s\r\n' % (vap.encode('ascii'), chains.encode('ascii')))
        if mode == '11b' or mode == '11g' or mode == '11a':
            rate_para = 'rate'
        elif mode == '11ng20' or mode == '11na20' or mode == '11ng40' or mode == '11na40':
            rate_para = 'mcs'
    ...
    def get_statistics(self):
        if int(channel) < 5000:
            ...
            vap = VAP_5G
        self.tn.read_until(b'root@OpenWrt:~# ', timeout=1)
        self.tn.write(b'iwpriv vap%s get_rx_info\r\n' % vap.encode('ascii'))
        command_result = self.tn.read_until(b'RX ERROR', timeout=2)
        logger.debug(command_result)
        rx_success = re.findall(b'OK:(\d+)', command_result)[0].decode('utf-8')
            ...
            if item == 'TX':
                ...
                if AUTO_ADJUST_POWER == '1':
                    ...
                    if avg_power == 'NA' or float(avg_power) > 99.000:
                        ...
                        if float(targetpower) <= float(avg_power) <= float(max_power):
                        ...
                        # power is not nomal
                            ...
                            while min_adj < 0 or max_adj > 0:
                                ...
                                if avg_power > max_power:
                                    ...
                                    adj = 1
                                c = c + 1
                                logger.debug('NEW VALUE(HEX):' + str(pwr_paras))
                ...
                if gen_txmax_report == 1:
                    # GEN REPORT
                    txmax_result_name = sn + '_' + 'TX_MAX_Result' + '_' + now_time + '.csv'
                    with open('./Result/' + txmax_result_name, 'w', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow(['FREQ', 'DATA_RATE', 'CHAIN', 'TX_POWER', 'POWER', 'GAIN', 'LIMIT',
                                         'RESULT', 'EVM', 'LIMIT', 'RESULT', 'FREQ_ERROR', 'LIMIT', 'RESULT',
                                         'LOLEAKAGE', 'LIMIT', 'RESULT', 'OBW', 'LIMIT', 'RESULT',
                ...
                avg_power, result_evm, result_symbol_clock_error, result_lo_leakage, result_mask = \
                    iq_wanted.get_data(pwr_len, txq_len, data_pwr, data_txq, mode, channel, rate, chain, 
                                       txmax_result_name, targetpower, spec_pwr, gain, spec_evm, evm_margin, 
                                       spec_symbol_clock_error, spec_lo_leakage, spec_mask, spec_obw_20M, 
                                       spec_obw_40M, spec_obw_80M, spec_obw_160M)
                if avg_power == 'NA' or result_evm == 'NA':
                    ...
                    logger.info('Default Measurer Power: ' + avg_power + ' Target Power: ' + targetpower)
                    delta_power = float(avg_power) - targetpower
                    ...
                    while delta_power < -2.5 and adjust_init_power_counts <= 10:
                        logger.info(Fore.RED + 'The rate\'s power is too low! Try to adjust to target power' + Style.RESET_ALL)
                        ...
                        iq_wanted.analysis()
                        pwr_len, txq_len, data_pwr, data_txq = iq_wanted.get_status()
                        avg_power = iq_wanted.get_data(pwr_len, txq_len, data_pwr, data_txq, mode, channel, rate, chain,
                                               txmax_result_name, targetpower, spec_pwr, gain, spec_evm, evm_margin,
                                               spec_symbol_clock_error, spec_lo_leakage, spec_mask, spec_obw_20M,
                                               spec_obw_40M, spec_obw_80M, spec_obw_160M)
                        delta_power = float(avg_power) - targetpower
                    ...
                    # power is ok, ADJUST
                    adjust_delta = 2
                    pwr_paras = init_value + adjust_delta
                    pwr_paras = hex(pwr_paras)
                    pwr_paras = re.sub('0x', '', pwr_paras)
                    get_power = []
                    get_delta_power = []
                    ...
                    while result_evm == 'Pass' and result_symbol_clock_error == 'Pass' and result_lo_leakage == 'Pass' \
                        ...
                        iq_wanted.read_data()
                        avg_power, result_evm, result_symbol_clock_error, result_lo_leakage, result_mask = \
                            iq_wanted.get_data(mode, rate, channel, chain, txmax_result_name, targetpower, spec_pwr,
                                               spec_evm, evm_margin, spec_symbol_clock_error, spec_lo_leakage, 
                                               spec_mask, spec_obw_20M, spec_obw_40M, spec_obw_80M)
                        get_power.append(avg_power)
                        logger.debug('Measurer Power List: ' + get_power)
                        delta_power = float(avg_power) - targetpower
                        delta_power = '{:.3f}'.format(delta_power)
                ...
                if int(sens) < int(sens_spec) and float(per) <= float(per_spec):
                    result = 'Pass'
                else:
                    result = 'Fail'
                ...
                rate_t = re.sub('\.', '_', rate_t)
                targetpower = eval('target_pwr_' + rate_t)
                spec_evm = eval('target_evm' + rate_t)
