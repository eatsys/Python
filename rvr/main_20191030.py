...
"""the main control, get the configure for entire test system
    2019.8.8 增加switch控制， STA类型还未使用，暂注释
             空口及传导测试类型已增加
    2019.10.30 拆分rssi查询 ap和sta分开组合
    """
...
from data.parameters import HOST_IP, AP_TYPE, RADIO, USER_NAME, PASSWORD, ANGLE_NUM, COM, STA_ADDRESS, \
    STA_TYPE, STA_USERNAME, STA_PASSWORD, STA_SWITCHIP, STA_SWITCHPORT, ATTENUATE_LIST, ATTEN_NUM, CHANNEL, ANGLE_LIST
...
from rssi_product import product_RSSI, product_RSSI_ssh
...
def test():
    ...
    logging.warning('When the test start, it will delete the result file,Please transport your test result firstly!')
    ...
    if not isExists_if:
        ...
        os.makedirs(ixchraiot_file)
    # set swivel table
    wait_table_time = set_swivel_table(ANGLE_NUM, COM, 'clockwise')
    # # set sta type
    ...
    for i in ATTENUATE_LIST:
        ...
        if ANGLE_NUM > 1 and swivel_count == 0:
            ...
            wait_table_time = set_swivel_table(ANGLE_NUM, COM, 'counter')
            Controller()
        ...
        for x in ANGLE_LIST:
            ...
            if AP_TYPE == 'WF-194' and STA_TYPE == 'WirelessCard':
                ...
                P_sta_rssi = product_RSSI(HOST_IP, USER_NAME, PASSWORD, RADIO, AP_TYPE)
                ...
                def get_statistics_tx():
                    # get  info
                    P_ap = product_RSSI(HOST_IP, USER_NAME, PASSWORD, RADIO, AP_TYPE)
                    ...
                    P_sta = product_RSSI(STA_ADDRESS, STA_USERNAME, STA_PASSWORD, RADIO, AP_TYPE)
                ...
                def get_statistics_tx():
                    # get AP's RSSI and link rate info
                    P_ap_counts = product_RSSI(HOST_IP, USER_NAME, PASSWORD, RADIO, AP_TYPE)
                ...
                def get_statistics_rx():
                    ...
                    P_ap_rssi = product_RSSI(HOST_IP, USER_NAME, PASSWORD, RADIO, AP_TYPE)
