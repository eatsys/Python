...
posY = 2
posZ = posY + 7
ws1.merge_cells(str(chr(posX) + str(posY) + ":" + chr(posX) + str(posZ)))
ws1[chr(posX) + str(posY)] = '6'
ws1[chr(posX) + str(posY)].font = cover_font
ws1["F1"]=1
ws1["F2"]=2
ws1["F3"]=3
posXX = ord('F')
...
posZ = 4
ws1["F4"] = "=SUM(1, 1)"
ws1["F5"] = '=SUM('+chr(posXX)+str(posY)+':'+chr(posXX)+str(posZ)+')'

ws2 = wb.create_sheet('Rate_over_Range', 2)
chart = LineChart()  #图表对象
chart.title = 'TX Graph'
chart.style = 13
chart.y_axis.title = 'Mbps'
chart.x_axis.title = 'RSSI(dBm)'
row_value = 2
for d in range(5):
    data = Reference(ws1, min_col=5, min_row=row_value) #涉及数据
...
ws2.add_chart(chart, "A6") #将图表添加到 sheet中
