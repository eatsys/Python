...
class dut():
    ...
    def login(self, username, password):
        # self.sn.read_until(b'Login: ', timeout=1)
        # self.sn.write(username.encode('ascii') + b'\n')
        # self.sn.read_until(b'Password:', timeout=1)
    ...
    def init(self, str1, str2, str3):
        ...
        #self.sn.write(b'iwpriv Hisilicon0 adjust_ppm -10' + b'\n')
    ...
    def set_default(self):
        ...
        self.sn.write(b'exit' + b'\n')
        self.sn.read_until(b'SU_WAP>', timeout=1)
        self.sn.write(b'wifi calibrate test parameter write wifichip 1 type power len 77 value default' + b'\n')
        time.sleep(2)
        self.sn.read_until(b'SU_WAP>', timeout=1)
        self.sn.write(b'shell' + b'\n')
        
        self.sn.write(b'wifi_equipment.sh init chip 1' + b'\n')
        time.sleep(2)
    ...
    def rx(self):
        ...
        logger.debug(channel )
        logger.debug(chain)
        self.sn.write(b'shell WifiRxReset.sh 1\r\n')
...
if __name__ == '__main__':
    ...
    for rows in dut_file:
        if rows[0] == 'log_enable':
            ...
            dut_com = rows[1]
            logger.info('DUT COM: '+ dut_com)
        elif rows[0] == 'Baudrate':
            baudrate = rows[1]
            logger.info('DUT Baudrate: '+ baudrate)
        ...
        elif rows[0] == '2gvap_value':
            vap_2g = rows[1]
            logger.info('2gvap_value: '+ vap_2g)
        elif rows[0] == '5gvap_value':
            vap_5g = rows[1]
            logger.info('5gvap_value: '+ vap_5g)
        ...
        if len(line) < 30 or line.startswith('//'):
            ...
            if item == 'TX':
                ...
                with open('./Result/' + rx_result_name, 'a+', newline='') as f2:
                    writer2 = csv.writer(f2)
                    writer2.writerow([channel, rate, chain, sens, int(int(per)/int(rx_packets))*100, result])
                logger.info('*************************************************************')
