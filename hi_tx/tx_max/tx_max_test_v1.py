...
class IQxel():
    ...
    def get_data(self, mode, channel, rate, chain, result_name, target_power, spec_pwr, spec_evm, evm_margin,
        ...
        with open('./Result/' + result_name, 'a+', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([result, channel, rate, chain, target_power, avg_power, spec_pwr, result_pwr, avg_evm,
...
if __name__ == '__main__':
    ...
    with open('./Result/' + result_name, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['All Test Results', 'FREQ', 'DATA_RATE', 'CHAIN', 'TX_POWER', 'POWER', 'LIMIT', 'RESULT',
        ...
        if not len(line) or line.startswith('//'):
            ...
            if avg_power == 'NA' or result_evm == 'NA':
                ...
                continue
                            ...
                            or result_mask == 'Fail':
                print(Fore.RED + 'The rate\'s TX Quality is Fail, Adjust quit' + Style.RESET_ALL)
                continue
            else:
                ...
                while delta_power < -2.5 and adjust_init_power_counts <= 10:
                    print(Fore.RED + 'The rate\'s power is too low! Try to adjust to target power' + Style.RESET_ALL)
